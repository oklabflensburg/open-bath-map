...
import RegionLandingPage from '../../components/seo/RegionLandingPage.vue'
...
const landingPage = computed(() => {
  const page = getLandingBySlug('region', slug.value)
  if (!page) {
...
const { data } = await useFetch<MapItemSearchResponse>(`${config.public.apiBase}/api/map/v1/items`, {
  ...
  key: `landing-region-items:${slug.value}`,
  server: true,
  lazy: false,
...
const canonicalUrl = computed(() => toAbsoluteUrl(getLandingPath('region', slug.value), siteUrl))
...
useJsonLd(() => buildLandingJsonLd(
  ...
  [
    { label: 'Start', to: '/' },
    { label: 'Regionen', to: '/regionen' },
    { label: landingPage.value.h1, to: getLandingPath('region', slug.value) },
  ],
  landingItems.value,
), `region-json-ld:${slug.value}`)
</script>
